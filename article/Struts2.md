...
> 每次请求时都会创建一个对应的 ActionContext 对象，请求处理完 ActionContext 销毁。

> 如何获取 ActionContext

struts2 的设计是，将 ActionContext 对象创建好之后，将 ActionContext 与当前线程绑定，需要获取 ActionContext 对象，只需要从 ThreadLocal 中获取即可。
...
> 注意 Action 中属性的名字应和 form 表中的数据一致。

### 属性驱动获得参数

> 直接在 Action 内部建立和参数名称相同的属性，然后设置 set、get 方法，就可以自动将表单数据获取到 Action 中的属性中了。
...
转载于：[Struts2 获取域和表单参数](https://www.jianshu.com/p/fd56e9634217)
